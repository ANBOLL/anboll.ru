document.addEventListener('DOMContentLoaded', () => {
    ...
    startBtn.addEventListener('click', async () => {
        ...
        progressLog.innerText = 'Начинается пиздец... \n По-братски подожди минуты 2...';
        ...
        const generateKeyProcess = async () => {
            ...
            for (let i = 0; i < 11; i++) {
                ...
                updateProgress(7 / keyCount, 'Типо чото важное происходит...');
        ...
        if (keys.length > 1) {
            keysList.innerHTML = keys.filter(key => key).map(key =>
                `<div class="key-item">
                    <input type="text" value="${key}" readonly>
                    <button class="copyKeyBtn" data-key="${key}">Скопировать</button>
                </div>`
            ).join('');
            ...
            keysList.innerHTML =
                `<div class="key-item">
                    ...
                    <button class="copyKeyBtn" data-key="${keys[0]}">Скопировать</button>
                </div>`;
        }
